...
if __name__ == "__main__":
    ...
    if args.num_point != 5000:
        exps_folder = exps_folder + "_num_point_{}".format(args.num_point)
    if args.i is not None:
        exps_folder = exps_folder + "_{}".format(args.i)
    ...
    model_path = base_path / "model_epochs_{:08d}.pth".format(args.epoch-1)
    ...
    for tar_folder in [Path("./data/DFaust_67_val/"), Path("./data/MPI_Limits/")]:
        print(
            "epoch={}, aug_type={}, kinematic_cond={} tar_folder.name={} EPN_layer_num={} EPN_input_radius={}".format(
                args.epoch, args.aug_type, args.kinematic_cond, tar_folder.name, args.EPN_layer_num, args.EPN_input_radius
            )
        )
        v2v = {}
        joint_err = {}
        ...
        print(
            "v2v={:.3f} joint_err={:.3f} part_acc={:.3f}".format(
                np.mean(list(v2v.values())), 
                np.mean(list(joint_err.values())), 
                100*np.mean(list(acc.values()))
            )
        )